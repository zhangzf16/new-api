				...
				if modelName, ok := taskData["model"].(string); ok && modelName != "" {
					...
					if hasRatioSetting && modelRatio > 0 {
						...
						if group != "" {
							...
							// 计算实际应扣费额度: totalTokens * billingModelRatio * groupRatio
							actualQuota := int(float64(taskResult.TotalTokens) * common.ApplyModelRatioBillingSurcharge(modelRatio) * finalGroupRatio)
